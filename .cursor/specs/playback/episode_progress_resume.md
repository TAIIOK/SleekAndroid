...
1. [x] Detail resume state (anime + Lampa) derives season/episode and progress from the same unfinished progress row (`0.01 < progress < 0.98`, latest `updatedAt`).
2. [x] Library `lastEpisode` / `lastWatchingEpisode` is used only as a fallback when no unfinished progress rows exist.
3. [x] Resume UI never pairs one episode’s number with `max(progress)` from another episode.
4. [x] Continue Watching includes only unfinished rows (`0.01 < progress < 0.98`, not `completed`).
